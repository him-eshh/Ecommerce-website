...
export default function OrderDetails() {
    ...
    async function extractOrderDetail() {
        const res = await getOrderDetail(params["order-details"]);
    ...
    return (
        <div className={`py-14 px-4 md:px-6 ${admindesign.orderstatus}`}>
            <div className={`flex justify-start items-start space-y-2 flex-col ${admindesign.adminorderinfoh2}`}>
            ...
            <div className={`mt-10 flex flex-col justify-center xl:flex-row items-stretch w-full xl:space-x-8 md:space-y-6 xl:space-y-0 ${admindesign.adminorderinfo}`}>
                <div className={`flex flex-col justify-start items-start w-full space-y-4 md:space-y-6 xl:space-y-8 `}>
                    <div className={`flex flex-col justify-start items-start bg-gray-50 px-4 py-4 md:p-6 xl:p-8 w-full `}>
                        <p className={`font-bold text-lg ${design.orderDetailsSummaryTitle}`}>Your order summary</p>
                        {orderDetails &&
                        ...
                        <div className={`flex justify-center md:justify-start xl:flex-col flex-col md:space-x-6 lg:space-x-8 xl:space-x-0 space-y-4 md:space-y-0 xl:space-y-12 md:flex-row items-center md:items-start `}>
                            <div className={`flex justify-center md:justify-start items-center md:items-start flex-col space-y-4 xl:mt-8`}>
                                ...
                                <p className={`text-base `}>
                                    Address: {orderDetails && orderDetails.shippingAddress.address}
                                </p>
                                <p className={`text-base `}>City: {orderDetails && orderDetails.shippingAddress.city}</p>
                                <p className={`text-base `}>
                                    Country: {orderDetails && orderDetails.shippingAddress.country}
                                </p>
                                <p className={`text-base`}>
                                    Postal Code: {orderDetails && orderDetails.shippingAddress.postalCode}
                                </p>
                            </div>
                        </div>
                        <button
                            onClick={() => router.push(`/`)}
                            className={`mt-5 mr-5  inline-block bg-black text-white px-5 py-3 text-xs font-medium uppercase tracking-wide ${buttondesign.buttonprimary}`}
                        >
                            Shop Again
                        </button>
                    </div>
